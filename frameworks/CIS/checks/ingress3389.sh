...
function checkSSHPort {
	...
	if [[ -z SG_TOTAL ]]; then
		...
		Name=ip-permission.from-port,Values=3389 Name=ip-permission.to-port,Values=3389 \
...
function performCheck {
	...
	echo "
CHECK:			No security groups should allow incoming traffic from 0.0.0.0/0 to Port 3389
SEVERITY:		HIGH
TYPE:			$TYPE
STATUS:			$STATUS
...
MITIGATION:		Every misconfigured security group needs to restrict the ingress for 0.0.0.0/0 on Port 3389.
			Removing connectivity to remote console services like RDP reduces the risk of unauthorized remote access.
" >> $RESULT_DIR
